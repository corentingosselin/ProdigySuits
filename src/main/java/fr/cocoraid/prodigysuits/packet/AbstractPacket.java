...
public abstract class AbstractPacket {
	...
	public void sendNearbyPacket(Location location) {
		Bukkit.getOnlinePlayers()
				.stream()
				.filter(cur -> cur.getWorld().equals(location.getWorld()))
				.filter(cur -> cur.getLocation().distanceSquared(location) <= viewDistance)
				.forEach(cur -> {
					sendPacket(cur);
				});
	}

    public void sendWorldPacket(World world) {
        Bukkit.getOnlinePlayers()
                .stream()
                .filter(cur -> cur.getWorld().equals(world))
                .forEach(cur -> {
                    sendPacket(cur);
                });
    }
